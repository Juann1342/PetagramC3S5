...
public class BaseDatos extends SQLiteOpenHelper {
    ...
    public ArrayList<Mascota> obtenerTodasLasMascotas() {
        ...
        while (registros.moveToNext()){
            ...
            if (registrosLikes.moveToNext()){
                mascotaActual.setRating(String.valueOf(registrosLikes.getInt(0)));
            }else {
    ...
    public ArrayList<Mascota> obtenerUltimas() {
        ...
        String query = "SELECT DISTINCT mascota.id,mascota.nombre,mascota.foto," +
                "mascota_likes.id_mascota FROM mascota,mascota_likes " +
                "WHERE mascota.id = mascota_likes.id_mascota " +
                "Order by (mascota_likes.id) DESC LIMIT 5";
